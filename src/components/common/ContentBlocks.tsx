...
export function Theorem({ title, children, proof, className = '' }: TheoremProps) {
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500/10 via-dark-800/80 to-dark-800/60 backdrop-blur border border-amber-500/20 p-5 transition-all duration-300 hover:border-amber-500/40 hover:shadow-lg hover:shadow-amber-500/5 ${proof ? 'cursor-pointer' : ''} ${className}`}
      onDoubleClick={proof ? () => setShowProof(!showProof) : undefined}
    >
      {/* Decorative gradient orb */}
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between">
              ...
              {proof && !showProof && (
                <span className="text-xs text-dark-500 italic">Double-click for proof</span>
              )}
            </div>
            <h4 className="font-semibold text-dark-100">{title}</h4>
          </div>
        ...
        {proof && (
          <div
            className={`overflow-hidden transition-all duration-500 ease-in-out ${
              showProof ? 'max-h-[2000px] opacity-100 mt-4' : 'max-h-0 opacity-0'
            }`}
          >
            <div className="pl-11 pt-4 border-t border-amber-500/20">
